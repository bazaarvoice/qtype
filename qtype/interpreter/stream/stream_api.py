...
def create_stream_flow_endpoint(app: FastAPI, flow: Flow) -> None:
    ...
    async def handle_question_stream(request: SimpleQuestionRequest) -> StreamingResponse:
        try:
            raw_question = request.prompt if request else None
            if not raw_question or not raw_question.strip():
                raise ValueError(
                    "A question is required in field 'prompt'."
                )

            flow_copy = flow.model_copy(deep=True)

            question_var = next(
                (var for var in flow_copy.inputs if getattr(var, "id", "") == "question"),
                None,
            )
            ...
            logger.error("Unexpected error preparing question stream: %s", exc, exc_info=True)
