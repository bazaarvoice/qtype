...
def resolve_semantic_ir(dsl_spec: dsl.QTypeSpec) -> ir.QTypeSpec:
    ...
    ir_prompts = _resolve_prompts(dsl_spec.prompts or [], ir_inputs, ir_outputs)
    ...
    _resolve_retriever_prompts(ir_retrievers, ir_prompts, dsl_spec.retrievers or [])
    ...
    ir_auth = list(dsl_spec.auth or [])  # AuthorizationProvider is imported directly
...
def _resolve_models(dsl_models: List[dsl.Model], lookup_maps: Dict[str, Dict[str, Any]]) -> List[ir.Model]:
    """Resolve DSL models to IR models."""
    ir_models = []
    for model in dsl_models:
        if isinstance(model, dsl.EmbeddingModel):
            ir_models.append(ir.EmbeddingModel(
                id=model.id,
...
def _resolve_outputs(dsl_prompts: List[dsl.Prompt], dsl_flows: List[dsl.Flow]) -> List[ir.Output]:
    ...
    for prompt in dsl_prompts:
        ...
        for input_id in prompt.input_vars:
            if input_id not in input_map:
                raise IRResolutionError(f"Input '{input_id}' not found for prompt '{prompt.id}'")
        ...
        for output_id in prompt.output_vars or []:
            if output_id not in output_map:
                raise IRResolutionError(f"Output '{output_id}' not found for prompt '{prompt.id}'")
            resolved_outputs.append(output_map[output_id])
        
        ir_prompts.append(ir.Prompt(
            id=prompt.id,
            path=prompt.path,
...
def _resolve_retrievers(dsl_retrievers: List[dsl.BaseRetriever], ir_models: List[ir.Model]) -> List[ir.BaseRetriever]:
    ...
    for retriever in dsl_retrievers:
        if isinstance(retriever, dsl.VectorDBRetriever):
            ...
            if retriever.embedding_model not in model_map:
                raise IRResolutionError(f"Embedding model '{retriever.embedding_model}' not found")
            
            embedding_model = model_map[retriever.embedding_model]
            if not isinstance(embedding_model, ir.EmbeddingModel):
                raise IRResolutionError(f"Model '{retriever.embedding_model}' is not an EmbeddingModel")
            
            ir_retrievers.append(ir.VectorDBRetriever(
                id=retriever.id,
            ...
            ir_retrievers.append(ir.SearchRetriever(
                id=retriever.id,
                index=retriever.index,
                top_k=retriever.top_k,
...
def _resolve_retriever_prompts(ir_retrievers: List[ir.BaseRetriever], ir_prompts: List[ir.Prompt], dsl_retrievers: List[dsl.BaseRetriever]) -> None:
    """Second pass to resolve SearchRetriever query_prompt references."""
    ...
    for ir_retriever in ir_retrievers:
        if isinstance(ir_retriever, ir.SearchRetriever):
            ...
            if dsl_retriever and isinstance(dsl_retriever, dsl.SearchRetriever) and dsl_retriever.query_prompt:
                if dsl_retriever.query_prompt not in prompt_map:
                    raise IRResolutionError(f"Query prompt '{dsl_retriever.query_prompt}' not found for retriever '{ir_retriever.id}'")
                ir_retriever.query_prompt = prompt_map[dsl_retriever.query_prompt]
...
def _resolve_tool_providers(
    ...
    for tp in dsl_tool_providers:
        ...
        ir_tool_providers.append(ir.ToolProvider(
            id=tp.id,
            name=tp.name,
...
def _resolve_memory(dsl_memory: List[dsl.Memory], ir_models: List[ir.Model]) -> List[ir.Memory]:
    ...
    for mem in dsl_memory:
        if mem.embedding_model not in model_map:
            raise IRResolutionError(f"Embedding model '{mem.embedding_model}' not found")
        
        embedding_model = model_map[mem.embedding_model]
        if not isinstance(embedding_model, ir.EmbeddingModel):
            raise IRResolutionError(f"Model '{mem.embedding_model}' is not an EmbeddingModel")
        
        ir_memory.append(ir.Memory(
            id=mem.id,
            type=mem.type,
            embedding_model=embedding_model,
            persist=mem.persist,
...
def _resolve_feedback(dsl_feedback: List[dsl.Feedback], ir_prompts: List[ir.Prompt]) -> List[ir.Feedback]:
    ...
    for feedback in dsl_feedback:
        ...
        if feedback.prompt:
            if feedback.prompt not in prompt_map:
                raise IRResolutionError(f"Prompt '{feedback.prompt}' not found for feedback '{feedback.id}'")
            prompt_obj = prompt_map[feedback.prompt]
        
        ir_feedback.append(ir.Feedback(
            id=feedback.id,
            type=feedback.type,
...
def _resolve_flows(
    ...
    for flow in dsl_flows:
        ...
        for input_id in flow.inputs or []:
            if input_id not in input_map:
                raise IRResolutionError(f"Input '{input_id}' not found for flow '{flow.id}'")
            resolved_inputs.append(input_map[input_id])
        ...
        for output_id in flow.outputs or []:
            if output_id not in output_map:
                raise IRResolutionError(f"Output '{output_id}' not found for flow '{flow.id}'")
            resolved_outputs.append(output_map[output_id])
        ...
        for mem_id in flow.memory or []:
            if mem_id not in memory_map:
                raise IRResolutionError(f"Memory '{mem_id}' not found for flow '{flow.id}'")
    ...
    for i, flow in enumerate(dsl_flows):
        ...
        for step in flow.steps:
            if isinstance(step, str):
                ...
                # Step object
                resolved_step = _resolve_step(step, input_map, output_map, prompt_map, tool_map, retriever_map, flow_map)
            ...
            for condition in flow.conditions:
                ...
                for step_id in condition.then:
                    ...
                    if not step_found and step_id in flow_map:
                        ...
                        step_found = True
                    if not step_found:
                        raise IRResolutionError(f"Step or flow '{step_id}' not found for condition")
                ...
                resolved_conditions.append(ir.Condition(
...
def _resolve_step(
    ...
    for input_id in dsl_step.input_vars or []:
        if input_id not in input_map:
            raise IRResolutionError(f"Input '{input_id}' not found for step '{dsl_step.id}'")
        resolved_inputs.append(input_map[input_id])
    ...
    for output_id in dsl_step.output_vars or []:
        if output_id not in output_map:
            raise IRResolutionError(f"Output '{output_id}' not found for step '{dsl_step.id}'")
        resolved_outputs.append(output_map[output_id])
    ...
    if dsl_step.component:
        ...
        if dsl_step.component in prompt_map:
            ...
            raise IRResolutionError(f"Component '{dsl_step.component}' not found for step '{dsl_step.id}'")
