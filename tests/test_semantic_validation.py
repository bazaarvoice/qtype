...
class TestUniqueIDs(unittest.TestCase):
    ...
    def test_unique_prompt_ids(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            prompts=[
                ...
                Prompt(id="prompt1", template="Hi", input_vars=["input1"]),  # Duplicate ID
            ]
        )
    ...
    def test_unique_memory_ids(self) -> None:
        ...
        spec = QTypeSpec(
            version="1.0",
            models=[EmbeddingModel(id="embed1", provider="openai", dimensions=1536)],
            memory=[
                Memory(id="mem1", type=MemoryType.vector, embedding_model="embed1"),
                Memory(id="mem1", type=MemoryType.vector, embedding_model="embed1"),  # Duplicate ID
            ]
        )
    ...
    def test_unique_tool_ids_within_provider(self) -> None:
        ...
        self.assertIn("Duplicate Tool.id tool1 in ToolProvider provider1", str(cm.exception))
    ...
    def test_unique_tool_provider_ids(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            tools=[
                ...
                ToolProvider(id="provider1", name="Provider 2", tools=[]),  # Duplicate ID
            ]
        )
        ...
        self.assertIn("Duplicate ToolProvider.id: provider1", str(cm.exception))
    ...
    def test_unique_auth_provider_ids(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            auth=[
                ...
                AuthorizationProvider(id="auth1", type="oauth2"),  # Duplicate ID
            ]
        )
        ...
        self.assertIn("Duplicate AuthorizationProvider.id: auth1", str(cm.exception))
    ...
    def test_unique_feedback_ids(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            feedback=[
                ...
                Feedback(id="feedback1", type=FeedbackType.STAR),  # Duplicate ID
            ]
        )
    ...
    def test_unique_retriever_ids(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            models=[EmbeddingModel(id="embed1", provider="openai", dimensions=1536)],
            retrievers=[
                VectorDBRetriever(id="ret1", index="index1", embedding_model="embed1"),
    ...
    def test_unique_step_ids_within_flow(self) -> None:
        ...
        spec = QTypeSpec(
            version="1.0",
            inputs=[Input(id="input1", type=VariableType.text)],
            prompts=[Prompt(id="prompt1", template="Hello", input_vars=["input1"])],
            flows=[
                Flow(
                    id="flow1",
                    ...
                    steps=[
                        ...
                        Step(id="step1", component="prompt1"),  # Duplicate step ID in same flow
                    ]
                )
            ]
        )
        ...
        self.assertIn("Duplicate Step.id step1 in Flow flow1", str(cm.exception))
    ...
    def test_unique_output_vars(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            inputs=[Input(id="input1", type=VariableType.text)],
            prompts=[
                Prompt(id="prompt1", template="Hello", input_vars=["input1"], output_vars=["output1"]),
                Prompt(id="prompt2", template="Hi", input_vars=["input1"], output_vars=["output1"]),  # Duplicate output
            ]
        )
...
class TestReferentialIntegrity(unittest.TestCase):
    ...
    def test_prompt_input_vars_reference_valid_inputs(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            inputs=[Input(id="input1", type=VariableType.text)],
            prompts=[
                Prompt(id="prompt1", template="Hello", input_vars=["input1", "invalid_input"])
            ]
        )
        ...
        self.assertIn("Prompt prompt1 input_var invalid_input not found in Input.id", str(cm.exception))
    ...
    def test_prompt_output_vars_reference_valid_outputs(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            inputs=[Input(id="input1", type=VariableType.text)],
            prompts=[
                Prompt(id="prompt1", template="Hello", input_vars=["input1"], output_vars=["invalid_output"])
            ]
        )
        ...
        self.assertIn("Prompt prompt1 output_var invalid_output not found in Output.id", str(cm.exception))
    ...
    def test_step_component_references_valid_retriever(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            models=[EmbeddingModel(id="embed1", provider="openai", dimensions=1536)],
            retrievers=[
                VectorDBRetriever(id="retriever1", index="index1", embedding_model="embed1")
            ],
            flows=[
                Flow(
                    id="flow1",
    ...
    def test_step_component_references_invalid_component(self) -> None:
        ...
        self.assertIn("Step step1 component invalid_component not found in any valid component type", str(cm.exception))
    ...
    def test_step_input_vars_reference_valid_inputs(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            flows=[
                Flow(
                    id="flow1",
                    mode=FlowMode.complete,
                    steps=[Step(id="step1", component="prompt1", input_vars=["invalid_input"])]
                )
            ]
        )
        ...
        self.assertIn("Step step1 input_var invalid_input not found in Input.id", str(cm.exception))
    ...
    def test_step_output_vars_reference_valid_outputs(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            flows=[
                Flow(
                    id="flow1",
                    mode=FlowMode.complete,
                    steps=[Step(id="step1", component="prompt1", output_vars=["invalid_output"])]
                )
            ]
        )
        ...
        self.assertIn("Step step1 output_var invalid_output not found in Output.id", str(cm.exception))
    ...
    def test_retriever_embedding_model_reference(self) -> None:
        ...
        spec = QTypeSpec(
            version="1.0",
            retrievers=[
                VectorDBRetriever(id="retriever1", index="index1", embedding_model="invalid_model")
            ]
        )
        ...
        self.assertIn("Retriever retriever1 embedding_model invalid_model not found in Model.id", str(cm.exception))
    ...
    def test_memory_embedding_model_reference(self) -> None:
        ...
        spec = QTypeSpec(
            version="1.0",
            memory=[
                Memory(id="mem1", type=MemoryType.vector, embedding_model="invalid_model")
            ]
        )
        ...
        self.assertIn("Memory mem1 embedding_model invalid_model not found in Model.id", str(cm.exception))
    ...
    def test_tool_provider_auth_reference(self) -> None:
        ...
        spec = QTypeSpec(
            version="1.0",
            tools=[
                ToolProvider(id="provider1", name="Test Provider", tools=[], auth="invalid_auth")
            ]
        )
        ...
        self.assertIn("ToolProvider provider1 auth invalid_auth not found in AuthorizationProvider.id", str(cm.exception))
    ...
    def test_flow_memory_reference(self) -> None:
        ...
        spec = QTypeSpec(
            version="1.0",
            flows=[
                Flow(id="flow1", mode=FlowMode.chat, steps=[], memory=["invalid_memory"])
            ]
        )
        ...
        self.assertIn("Flow flow1 memory invalid_memory not found in Memory.id", str(cm.exception))
...
class TestFlowValidation(unittest.TestCase):
    ...
    def test_flow_steps_string_references_valid_flow(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            flows=[
                Flow(id="flow1", mode=FlowMode.complete, steps=[]),
                Flow(id="flow2", mode=FlowMode.complete, steps=["flow1"])  # Valid reference
            ]
        )
    ...
    def test_flow_steps_string_references_invalid_flow(self) -> None:
        ...
        spec = QTypeSpec(
            version="1.0",
            flows=[
                Flow(id="flow1", mode=FlowMode.complete, steps=["invalid_flow"])
            ]
        )
        ...
        self.assertIn("Flow flow1 steps[] string invalid_flow not found in Flow.id", str(cm.exception))
    ...
    def test_flow_inputs_reference_valid_inputs(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            inputs=[Input(id="input1", type=VariableType.text)],
            flows=[
                Flow(id="flow1", mode=FlowMode.complete, steps=[], inputs=["input1", "invalid_input"])
            ]
        )
        ...
        self.assertIn("Flow flow1 input invalid_input not found in Input.id", str(cm.exception))
    ...
    def test_flow_outputs_reference_valid_outputs(self) -> None:
        ...
        spec = QTypeSpec(
            version="1.0",
            inputs=[Input(id="input1", type=VariableType.text)],
            prompts=[Prompt(id="prompt1", template="Hello", input_vars=["input1"], output_vars=["output1"])],
            flows=[
                Flow(id="flow1", mode=FlowMode.complete, steps=[], outputs=["output1", "invalid_output"])
            ]
        )
        ...
        self.assertIn("Flow flow1 output invalid_output not found in Output.id", str(cm.exception))
    ...
    def test_flow_condition_then_references_valid_steps_or_flows(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            flows=[
                Flow(
                    ...
                    steps=[Step(id="step1", component="prompt1")],
                    conditions=[
                        Condition(if_var="input1", equals="test", then=["step1", "invalid_ref"])
                    ]
                )
            ]
        )
        ...
        self.assertIn("Flow flow1 condition then[] invalid_ref not found in Step.id or Flow.id", str(cm.exception))
    ...
    def test_flow_condition_else_references_valid_steps_or_flows(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            flows=[
                Flow(
                    ...
                    steps=[Step(id="step1", component="prompt1")],
                    conditions=[
                        Condition(if_var="input1", equals="test", then=["step1"], else_=["invalid_ref"])
                    ]
                )
            ]
        )
        ...
        self.assertIn("Flow flow1 condition else_[] invalid_ref not found in Step.id or Flow.id", str(cm.exception))
    ...
    def test_chat_mode_flow_must_have_memory(self) -> None:
        ...
        self.assertIn("Flow flow1 mode=chat must set memory[]", str(cm.exception))
    ...
    def test_non_chat_mode_flow_must_not_have_memory(self) -> None:
        ...
        spec = QTypeSpec(
            version="1.0",
            models=[EmbeddingModel(id="embed1", provider="openai", dimensions=1536)],
            memory=[Memory(id="mem1", type=MemoryType.vector, embedding_model="embed1")],
            flows=[
                Flow(id="flow1", mode=FlowMode.complete, steps=[], memory=["mem1"])  # Memory set on non-chat
            ]
        )
        ...
        self.assertIn("Flow flow1 mode!=chat must not set memory[]", str(cm.exception))
...
class TestMemoryVsRetriever(unittest.TestCase):
    ...
    def test_memory_id_not_used_as_step_component(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            flows=[
                Flow(
                    id="flow1",
                    mode=FlowMode.complete,
                    steps=[Step(id="step1", component="mem1")]  # Memory used as component
                )
            ]
        )
        ...
        self.assertIn("Memory.id mem1 used as Step.component in Flow flow1", str(cm.exception))
...
class TestToolingRules(unittest.TestCase):
    ...
    def test_tool_names_unique_within_provider(self) -> None:
        ...
        self.assertIn("Duplicate Tool.name duplicate_name in ToolProvider provider1", str(cm.exception))
    ...
    def test_tool_must_have_input_and_output_schema(self) -> None:
        ...
        self.assertIn("Tool tool1 in ToolProvider provider1 missing input_schema or output_schema", str(cm.exception))
...
class TestModelEmbeddingRules(unittest.TestCase):
    ...
    def test_embedding_model_must_not_have_inference_params(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            models=[
                EmbeddingModel(
                    ...
                    provider="openai",
                    dimensions=1536,
                    inference_params={"temperature": 0.7}  # Should not be allowed
                )
            ]
        )
        ...
        self.assertIn("EmbeddingModel embed1 must not have inference_params", str(cm.exception))
...
class TestPromptRequirements(unittest.TestCase):
    ...
    def test_prompt_must_define_template_or_path_not_both(self) -> None:
        ...
        self.assertIn("Prompt prompt1 must define either template or path, but not both", str(cm.exception))
...
class TestCircularFlowDetection(unittest.TestCase):
    ...
    def test_circular_reference_in_flows(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            flows=[
                ...
                Flow(id="flow2", mode=FlowMode.complete, steps=["flow1"]),  # Circular reference
            ]
        )
        ...
        self.assertTrue(
            "Circular reference detected in Flow flow1" in error_str or
            "Circular reference detected in Flow flow2" in error_str
        )
    ...
    def test_self_referencing_flow(self) -> None:
        ...
        spec = QTypeSpec(
            version="1.0",
            flows=[
                Flow(id="flow1", mode=FlowMode.complete, steps=["flow1"]),  # Self-reference
            ]
        )
        ...
        self.assertIn("Circular reference detected in Flow flow1", str(cm.exception))
    ...
    def test_complex_circular_reference(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            flows=[
                ...
                Flow(id="flow3", mode=FlowMode.complete, steps=["flow1"]),  # Creates circle: flow1 -> flow2 -> flow3 -> flow1
            ]
        )
        ...
        error_str = str(cm.exception)
        self.assertTrue(
            any(f"Circular reference detected in Flow flow{i}" in error_str for i in [1, 2, 3])
        )
...
class TestValidSpecExamples(unittest.TestCase):
    ...
    def test_complete_valid_spec(self) -> None:
        ...
        spec = QTypeSpec(
            ...
            prompts=[
                Prompt(id="prompt1", template="Hello {input1}", input_vars=["input1"], output_vars=["output1"]),
                Prompt(id="prompt2", path="/path/to/template", input_vars=["input2"], output_vars=["output2"])
            ],
            memory=[
                Memory(id="mem1", type=MemoryType.vector, embedding_model="embed1")
            ],
            ...
            flows=[
                Flow(
                    ...
                    memory=["mem1"],
                    steps=[
                        Step(id="step1", component="prompt1", input_vars=["input1"], output_vars=["output1"]),
                        Step(id="step2", component="tool1"),
                        Step(id="step3", component="ret1")
                    ],
                    conditions=[
                        Condition(if_var="input1", equals="test", then=["step1"], else_=["step2"])
                    ]
                ),
                Flow(
                    id="flow2",
                    ...
                    outputs=["output2"],
                    steps=[
                        Step(id="step4", component="prompt2", input_vars=["input2"], output_vars=["output2"]),
