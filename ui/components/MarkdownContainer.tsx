    ...
    <div className="bg-gray-50 dark:bg-gray-900 p-3 rounded">
      <ReactMarkdown
        ...
        components={{
          ...
          h2: ({ children }) => (
            <h2 className="text-lg font-semibold mb-3 mt-5 text-gray-900 dark:text-gray-100">
              {children}
            </h2>
          ),
          h3: ({ children }) => (
            <h3 className="text-base font-semibold mb-2 mt-4 text-gray-800 dark:text-gray-200">
              {children}
            </h3>
          ),
          h4: ({ children }) => (
            <h4 className="text-sm font-medium mb-2 mt-3 text-gray-700 dark:text-gray-300">
              {children}
            </h4>
          ),
          // Fix list styling to remove errant bullets
          ul: ({ children }) => (
            <ul className="mb-3 pl-0 space-y-1 text-gray-800 dark:text-gray-200">
              {children}
            </ul>
          ),
          ol: ({ children }) => (
            <ol className="mb-3 pl-0 space-y-1 text-gray-800 dark:text-gray-200">
              {children}
            </ol>
          ),
          li: ({ children }) => (
